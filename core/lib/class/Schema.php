...
class Schema
{
	...
	private static $pdo_link = false;

	public static function pdoLink($pdo_link)
	{
		self::$pdo_link = $pdo_link;
	}
	...
	public function execute($pdo_link=false)
	{
		if($pdo_link)
			self::pdoLink($pdo_link);

		$DB = (!self::$pdo_link) ? DB() : self::$pdo_link;
