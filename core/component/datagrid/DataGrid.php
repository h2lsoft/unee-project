...
class DataGrid {
	...
	public function addColumnBoolean(string $name, string $label='', bool $order_by=false, string $classes='', bool $editable=true):void
	{
		$this->addColumn($name, $label, $order_by, "min center {$classes}");
		$this->columns[$name]['type'] = 'boolean';
		$this->columns[$name]['editable'] = ($editable && $this->user_has_right_edit);


		// listener
		if(get('ajaxer') == 1 && get('ajaxer-action') == 'toggle' && get('id') && get('column') == $name)
		{
			$resp = ['error' => false, 'error_message' => ""];
			$id = get('id');
			$v = get('value');
			...
			DB($this->table)->update([$name => (($v == 0) ? 'yes' : 'no')], $id);
	...
	public function render()
	{
			...
			foreach($_GET['search'] as $vs)
			{
				...
				if(isset($this->search_columns[$col]))
				{
					...
					else
					{
						...
						elseif($vs[1] == 'empty')
						{
							$str = str_replace('[@operator_input]', " IN('') ", $str);
						}
						elseif($vs[1] == '!empty')
						{
							$str = str_replace('[@operator_input]', " NOT IN('') ", $str);
						}
						elseif($vs[1] == 'in' || $vs[1] == '!in')
						{
							$not = ($vs[1] == 'in') ? '' : 'NOT';
		...
		foreach($searches as $search)
		{
			if(str_starts_with($search, 'xTags||'))
			{
				$tag = str_erase('xTags||', $search);
				$tag = trim($tag);

				if(!empty($tag) && !in_array($tag, $searched_tags))
					$searched_tags[] = $tag;
			}
		}
