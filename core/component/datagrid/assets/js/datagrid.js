...
// toggle
$('body').on('click', '.datagrid .btn-action-toggle', function(e){

	e.preventDefault();
	e.stopPropagation();

	const $parent_link = $(this);
	const column_value = $(this).children('i').data('value');


	loaderShow();
	$.getJSON('?ajaxer=1&ajaxer-action=toggle&column='+$(this).data('column')+'&id='+$(this).data('id')+'&value='+column_value, function(response){

		if(response.error)return;

		if(column_value === 1)
		{
			$parent_link.children('i').data('value', 0).prop('class', "text-muted bi bi-circle");
		}
		else
		{
			$parent_link.children('i').data('value', 1).prop('class', "text-success bi bi-check-circle-fill");
		}
...
$('body').on('click', '.datagrid-search-offcanvas .btn-search-execute', function(e){
	...
	if($('.datagrid-search-offcanvas .datagrid_search_field').length)
	{
		...
		$('.datagrid-search-offcanvas .datagrid_search_field').each(function(){
			...
			if(v == '' && !$(this).find('.datagrid-search-input__value__'+f_type).is(':disabled'))
			{
...
$('body').on('click', '.xtag', function(e){

	e.preventDefault();

	let tag = $(this).text().trim();
	let paramToCheck = 'search[]=xTags||' + tag;
	let url = window.location.href;
	let params = new URLSearchParams(window.location.search);

	let existingTags = [];
	for (const [key, value] of params.entries('search[]')) {
		if (value.startsWith('xTags||')) {
			existingTags.push(value.slice(7));
		}
	}

	let tagsToRemove = existingTags.filter(existingTag => existingTag === tag);
	tagsToRemove.forEach(tagToRemove => {
		params.delete('search[]', 'xTags||' + tagToRemove);
	});

	if (!existingTags.includes(tag)) {
		params.append('search[]', 'xTags||' + tag);
	}
