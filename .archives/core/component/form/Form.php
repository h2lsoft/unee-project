...
class Form {
	...
	public function linkController(Controller $controller, int $id = 0):void
	{
		...
		if($id)
		{
			...
			$record = Db($this->table)->query("SELECT * FROM {$this->table} WHERE deleted = 'no' AND id = :id", [':id' => $this->id])->fetch();
	...
	public function addTagManager(string $signature='', bool $required=false, array $attributes=[])
	{
		$name = 'xTags';
	...
	public function addFileImage(
									string $name, string $label="", bool $required=false,
								 	string $upload_dir="", string $max_upload_size="",
									array $allowed_exts=['jpg','jpeg','png','apng','svg','webp','avif','gif', 'jxl'],
									array $allowed_mimes=[],
									string $help="",
									array $attributes=[]
								):Form
	...
	private function autoRulesCompile():void
	{
		...
		{
			if(in_array($this->fields[$field]['type'], ['text','radio', 'textarea', 'number', 'tel', 'password', 'email', 'select', 'switch', 'file-image', 'date', 'datetime', 'time']))
			{
				...
				if($this->fields[$field]['type'] == 'file-image')
				{
					...
					if(count($this->fields[$field]['mimes']))$this->validator->input($field, $label)->fileMime($this->fields[$field]['mimes']);
				}
	...
	public function save(array $exceptions=[], array $dynamic_fields=[]):array
	{
		global $app;
		...
		{
			if(isset($field['name']) && !in_array($field['name'], $this->exceptions) && in_array($field['type'], ['text', 'radio', 'textarea', 'select', 'number', 'switch', 'email', 'tel', 'date', 'datetime', 'time']))
			{
		...
		if(isset($this->fields['xTags']))
		{
			...
			foreach($tags as $tag)
			{
				...
				DB('xcore_tag')->insert(['signature' => $this->fields['xTags']['signature'], 'tag' => $tag, 'record_id' => $this->id]);
			}
		}
